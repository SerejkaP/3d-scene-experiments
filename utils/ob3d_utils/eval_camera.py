...
import utils.ob3d_utils.camera_metric as metric
...
def make_relative_pose(gt_se3, pred_se3):
    ...
    s, R, t = points_alignment(
        pred_se3[:, :3, 3].numpy(), gt_se3[:, :3, 3].numpy(), with_scale=True
    )
...
def evaluate_camera(
    gt_cameras_path, pred_cameras_path, pred_cameras_type, output_path=None
):
    ...
    rel_rangle_deg = metric.rotation_angle(
        relative_pose_gt[:, :3, :3], relative_pose_pred[:, :3, :3]
    )
    rel_tangle_deg = metric.translation_angle(
        relative_pose_gt[:, :3, 3], relative_pose_pred[:, :3, 3]
    )
